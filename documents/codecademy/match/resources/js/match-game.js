...
MatchGame.renderCards = function(cardValues, $game) {
  ...
  for (var cardIndex = 0; cardIndex < cardValues.length; cardIndex++) {
    var value = cardValues[cardIndex];
  ...
  var $objectCard = $('<div class="col-xs-3 card"> </div>');
  $objectCard.data(data);

  $game.append($objectCard);
  }
...
MatchGame.flipCard = function($card, $game) {
  ...
  if (flippedCards.length === 2) {
    if (flippedCards[0].data('value') === flippedCards[1].data('value')) {
      ...
      flippedCards[0].css(matchedColors);
      flippedCards[1].css(matchedColors);
    }
    else {
